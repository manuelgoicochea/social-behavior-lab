      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            ...
            <p className="text-gray-600 text-sm">{t('events.empty')}</p>
          </div>
        ) : (
          events.map(event => {
            const c = COLOR_MAP[event.color] || COLOR_MAP.gray
            // Traducir mensaje usando key+params si existe, si no usar message legacy
            const message = event.key ? t(event.key, event.params) : (event.message || '')
            return (
              <div
                key={event.id}
              ...
              >
                ...
                <p className={`text-xs leading-relaxed ${c.text}`}>{message}</p>
              </div>
            )
          })
        )}
